	...
	render() {
		...
		return (
			<Widget
				title={
					<h5>
						Your <span className="fw-semi-bold">Firmware Contracts</span>
					</h5>
				}
				close
							...
							<tr key={d.rootCid}>
								...
								<th>
									<span
										...
										onClick={() => downloadFirmwareBinary(d.rootCid, 'binary.bin')}
									/>
								</th>
								<th>
									<span
										className={'glyphicon glyphicon-remove'}
										onClick={() => removeFromFilecoin(d.rootCid).catch(alert)}
									/>
								</th>
							</tr>
						))}
